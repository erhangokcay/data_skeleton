function [clust] = STREAM_SKELETON_RESHUFFLE_DIST_VER4(clust, var, T)
                ...
                while ( modified )
                    ...
                    if ( len1 > 1 && len2 > 1 ) 
                        for k=1:len1
                           ...
                           xtemp2 = [ xy2 xy1(:,k)];
                            [DCirCefn,~,maxDistn,IntCefn] = EXECUTE_CPP_CEF_V1 ( xtemp1, var, 0, statInfo );
                            [DCirCefm,~,maxDistm,IntCefm] = EXECUTE_CPP_CEF_V1 ( xtemp2, var, 0, statInfo );
                           % If the clusters are tigther
                           % cond = lessT(DCirCefn,T) && lessT(DCirCefm,T);
                           if ( (maxDistn < clust(n).maxDist && maxDistm <= clust(m).maxDist) || ...
                                   (maxDistn <= clust(n).maxDist && maxDistm < clust(m).maxDist) ) 
                                clust(n).xy = xtemp1;
                                clust(m).xy = xtemp2;
