function [inp_str,inp_data] = getData_Pittsburgh_Bridges 
    fname = 'D:\WORKING\ACADEMIC\MATLAB\SKELETON\DataSets_Skeleton\Pittsburgh Bridges\bridges.data.version2';
    fid=fopen(fname,'r');
    % Missing Numeric Fields : ? --> 0 (As a category) 
    A=textscan(fid,'%s%s%f%s%s%s%s%s%s%s%s%s%s','Delimiter',',');
    size(A)
    Nrows = size(A{1},1);
    Ncols = size(A,2);
