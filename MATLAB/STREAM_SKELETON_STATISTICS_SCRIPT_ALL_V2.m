...
TList_DIABETIC_1=[1000,500,100,10,1,0.1, 10e-3,10e-4,10e-5,10e-8,10e-25,10e-50,10e-200,10e-350,10e-500];
...
for T=1:length(TList)
    ...
    for n=1:10
        disp(n);
        [outArr(n)] = STREAM_SKELETON_PERFORMANCE_ALL1 ( data_DIABETIC, 0.2, false, 0, cef_var, TList(T), TList(T));
    end
    % For debugging
    outAll(T).out = outArr;
    outAVEArr(T)=STREAM_SKELETON_AVE_ALL_V2(outArr);
end

stat_DIABETIC.cef_var = cef_var;
stat_DIABETIC.TList = TList_DIABETIC_1;
stat_DIABETIC.stat = outAVEArr;
