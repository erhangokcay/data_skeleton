function [inp_str,inp_data] = getData_Horse_Colic 
    fname = 'G:\WORKING\ACADEMIC\MATLAB\SKELETON\DataSets_Skeleton\Horse Colic\horse-colic_combined.data';
    fid=fopen(fname,'r');
    % Missing Numeric Fields : ? --> 0 (As a category) 
    A=textscan(fid,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%s%f%f%f%f','Delimiter',' ');
    size(A)
    Nrows = size(A{1},1);
    Ncols = size(A,2);
