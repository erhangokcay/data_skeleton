function [clust,numOfClust]=STREAM_SKELETON_VER4_nPNT_OPTIMIZED_V2_MEM_DEBUG1_1(xy,var,T1,T2,randlist)
...
%         end        
    ...
    [clust,numC]=CPP_Recluster(clust,numC,var,T2,statInfo) ;
...
%                 end                                
       ...
       statInfo1 = CEF_CALCULATE_STATINFO(size(clust(1).xy),var,0);
        ...
        while (true)
            ...
            for nn=1:numC
                for mm=nn+1:numC
                    ...
                    if ( ~(clust(nn).modified) && ~(clust(mm).modified))
                        continue
                    end
                    %disp(n)
                    %disp(m)
                    % [Dcef,Dmean,Dvar] = STREAM_STAT_WITHIN ([clust(n).xy,clust(m).xy], var, 0 );
                    if ( ~isempty(clust(nn).xy) && ~isempty(clust(mm).xy))
                        xymerged = [clust(nn).xy,clust(mm).xy];
                        if ( size(clust(nn).xy,2) == 1 &&  size(clust(mm).xy,2) == 1)                    
                            [cefMAXNew,cefSUMNew,outMAXPointList,outMAXDistNew, maxListNew, maxValueNew] = EXECUTE_CPP_SKELETON_V1_OPTIMIZED_V1 ( xymerged, 0, 0, 0, 0, 0, 0, var, 0, statInfo1, 0 );
                        ...
                        elseif ( size(clust(nn).xy,2) >=  size(clust(mm).xy,2) )        % size > 1

                            temp.xy = clust(nn).xy;
                            temp.DCirCef = clust(nn).DCirCef;
                            temp.maxDist = clust(nn).maxDist;
                            temp.IntCef = clust(nn).IntCef;
                            temp.outMAXPointList = clust(nn).outMAXPointList;
                            temp.maxListNew = clust(nn).maxListNew;
                            temp.maxValueNew = clust(nn).maxValueNew;
                            for h=1:size(clust(mm).xy,2)
                                if ( size(temp.maxListNew,2) == 0 )
                                    disp('4-2 MAXLIST ERROR')
                                    pause
                                else
                                    xyp = clust(mm).xy(:,h);
                                    [cefMAXNew,cefSUMNew,outMAXPointList,outMAXDistNew, maxListNew, maxValueNew] = EXECUTE_CPP_SKELETON_V1_OPTIMIZED_V1 ( temp.xy, temp.IntCef, temp.DCirCef, temp.maxValueNew, temp.maxListNew, xyp, 0, var, 0, statInfo1, 2 );
                                end
                                temp.xy = [temp.xy xyp];
                                temp.DCirCef = cefMAXNew;
                                temp.maxDist = outMAXDistNew;
                        ...
                        elseif ( size(clust(nn).xy,2) <=  size(clust(mm).xy,2) )        % size > 1
                            ...
                            temp.xy = clust(mm).xy;
                            temp.DCirCef = clust(mm).DCirCef;
                            temp.maxDist = clust(mm).maxDist;
                            temp.IntCef = clust(mm).IntCef;
                            temp.outMAXPointList = clust(mm).outMAXPointList;
                            temp.maxListNew = clust(mm).maxListNew;
                            temp.maxValueNew = clust(mm).maxValueNew;
                            for h=1:size(clust(nn).xy,2)
                                if ( size(temp.maxListNew,2) == 0 )
                                    disp('4-2 MAXLIST ERROR')
                                    pause
                                else
                        ...
                        if ( res )
                            clust(nn).xy = xymerged;

                            clust(nn).DCirCef = temp.DCirCef;
                            clust(nn).maxDist = temp.maxDist;
                            clust(nn).IntCef = temp.IntCef;
                            clust(nn).outMAXPointList = temp.outMAXPointList;
                            clust(nn).maxListNew = temp.maxListNew;
                            clust(nn).maxValueNew = temp.maxValueNew;

                            clust(nn).modified=true;
        ...
        for nn=1:numC
            if ( ~isempty(clust(nn).xy) )
                outK = outK + 1;
                clustOut(outK).xy = clust(nn).xy;
                clustOut(outK).DCirCef =  clust(nn).DCirCef;
                clustOut(outK).maxDist =  clust(nn).maxDist;
                clustOut(outK).IntCef =  clust(nn).IntCef;
                clustOut(outK).outMAXPointList =  clust(nn).outMAXPointList;
                clustOut(outK).maxListNew =  clust(nn).maxListNew;
                clustOut(outK).maxValueNew =  clust(nn).maxValueNew;
...
%         end
   ...
   [clust,numC]=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED_MEM_DEBUG1_1(clust,clustEmpty, numC, var,T2);
   %clust=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED_MEM_DEBUG1(clust,var,T2);
