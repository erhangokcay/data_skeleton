function [clust,changed] = STREAM_SKELETON_RESHUFFLE_DIST_VER5_nPNT_OPTIMIZED_FAST(clust, numC, var, nPnt, T)
            ...
            for m=n+1:numC
                ...
                while ( modified )
                    ...
                    for k=1:nPnt % # of closest points from clust1  
                        ...
                        len1 = size(xy1Temp,2);
                        if ( len1 > k ) 
                           [u1(k),u2(k)]=CPP_Skeleton_ClosestPoints_Fast(xy1Temp,xy2); 
                           xtemp1 = xy1;                         
                           xtemp1(:,u1(1:k)) = []; % Delete from clust1
                           xtemp2 = [ xy2 xy1(:,u1(1:k))]; % add to clust2
                           [cefMAXNew1,cefSUMNew1,outMAXPointList1,outMAXDistNew1, maxListNew1, maxValueNew1] = EXECUTE_CPP_SKELETON_V1_OPTIMIZED_V1_FAST ( xtemp1, 0, 0, 0, 0, 0, 0, var, 0, statInfo, 0 );
                           [cefMAXNew2,cefSUMNew2,outMAXPointList2,outMAXDistNew2, maxListNew2, maxValueNew2] = EXECUTE_CPP_SKELETON_V1_OPTIMIZED_V1_FAST ( xtemp2, 0, 0, 0, 0, 0, 0, var, 0, statInfo, 0 );
                           ...
                           if ( (outMAXDistNew1 < clust(n).maxDist && outMAXDistNew2 <= clust(m).maxDist) || ...
                                   (outMAXDistNew1 <= clust(n).maxDist && outMAXDistNew2 < clust(m).maxDist) ) 
                                clust(n).xy = xtemp1;
                                clust(m).xy = xtemp2;
