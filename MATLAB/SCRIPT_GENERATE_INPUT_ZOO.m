...
data_Zoo = getData_Zoo;
%TList_IRIS_1=[1000,500,100,10,1,0.1, 10e-3,10e-4,10e-5,10e-8,10e-25,10e-50,10e-200,10e-350,10e-500]
% T=0 means original dataset. Copy by hand, not in the loop
TList_Zoo_1=[1e-10,1e-20,1e-30,1e-50];
TList = TList_Zoo_1;
cef_var=0.1;
RdSelect = 1;
...
for T=1:length(TList)    
    for n=1:10
        ...
        inpstrZoo(T,n) = STREAM_SKELETON_INPUT_DATASET( data_Zoo, 0.2, false, [], 0.25, TList(T), TList(T) );
%        inpstrZoo(T,n) = STREAM_SKELETON_INPUT_DATASET_OPTIMIZED ( data_Zoo, 0.2, false, [], 0.1, TList(T), TList(T) );
        % [outArr(n)] = STREAM_SKELETON_PERFORMANCE_ALL1 ( data_Cardiotocography, 0.2, false, 0, 0.1, TList(T), TList(T));
    end    
    save MATLAB_Zoo_15_03_2021.mat
    % For debugging
