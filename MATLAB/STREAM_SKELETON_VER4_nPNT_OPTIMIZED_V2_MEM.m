function [clust,numOfClust]=STREAM_SKELETON_VER4_nPNT_OPTIMIZED_V2_MEM(xy,var,T1,T2,randlist)
...
%         end        
    ...
    [clust,numC]=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED_MEM(clust,clustEmpty,numC,var,T2) ;
    %[globalTmean, globalTvar]= STREAM_CLUSTER_STAT(clust,var); % this should be incremental
...
%        plot(xyp(1),xyp(2),'ro');
        ...
        for m=1:numC
            ...
            if ( size(clust(m).xy,2) == 1 )
                 disp('F1')
				 tic;[cefMAXNew,cefSUMNew,outMAXPointList,outMAXDistNew, maxListNew, maxValueNew] = EXECUTE_CPP_SKELETON_V1_OPTIMIZED_V1 ( [clust(m).xy,xyp], 0, 0, 0, 0, 0, 0, var, 0, statInfo, 0 );toc
                   ...
                   if ( size(clust(m).maxListNew,2) == 0 ) 
                        ...
                        pause
                         ...
                         disp('F2')
                         tic;[cefMAXNew,cefSUMNew,outMAXPointList,outMAXDistNew, maxListNew, maxValueNew] = EXECUTE_CPP_SKELETON_V1_OPTIMIZED_V1 ( clust(m).xy, clust(m).IntCef, clust(m).DCirCef, clust(m).maxValueNew, clust(m).maxListNew, xyp, 0, var, 0, statInfo, 2 );toc
                   end
            end	
...
%                 end                                
       ...
       %[globalTmean, globalTvar]= STREAM_CLUSTER_STAT(clust,var); % this should be incremental
       disp('F3')
       tic;[clust,numC]=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED_MEM(clust,clustEmpty, numC, var,T2);toc
       %fprintf('RES=%f \n',toc);
       %[globalTmean, globalTvar]= STREAM_CLUSTER_STAT(clust,var); % this should be incremental 
...
%         end
   ...
   [clust, c] = STREAM_SKELETON_RESHUFFLE_SINGLE_VER4_nPNT_OPTIMIZED(clust, var, 10e-25, T2);   
   clust=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED(clust,var,T2);
 
   [clust, c] = STREAM_SKELETON_RESHUFFLE_DIST_VER5_nPNT_OPTIMIZED(clust,var, 2, 0);
   [clust, c] = STREAM_SKELETON_RESHUFFLE_SINGLE_VER4_nPNT_OPTIMIZED(clust, var, 10e-25, T2);
   clust=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED(clust,var,T2); 
    
   [clust, c] = STREAM_SKELETON_RESHUFFLE_CEF_VER5_nPNT_OPTIMIZED(clust,var, 2, 0);
   [clust, c] = STREAM_SKELETON_RESHUFFLE_SINGLE_VER4_nPNT_OPTIMIZED(clust, var, 10e-25, T2);
   clust=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED(clust,var,T2); 

   %%% ??? 
   [clust,c] = STREAM_SKELETON_RESHUFFLE_MINDIST_VER5_nPNT_OPTIMIZED(clust,var, 2, 0);
   [clust, c] = STREAM_SKELETON_RESHUFFLE_SINGLE_VER4_nPNT_OPTIMIZED(clust, var, 10e-25, T2);
   clust=STREAM_SKELETON_RECLUSTER_VER3_OPTIMIZED(clust,var,T2); 
