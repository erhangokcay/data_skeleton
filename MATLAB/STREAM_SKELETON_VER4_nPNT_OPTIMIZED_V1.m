function [clust,numOfClust]=STREAM_SKELETON_VER4_nPNT_OPTIMIZED_V1(xy,var,T1,T2,randlist)
...
%         end        
    ...
    if ( isempty(list) )
        ...
        if ( length(list) == 1 ) % Join point to the group
            ...
            %clust(cn).Tvar = Dvar;
            clust(cn).DCirCef = cefMAXNew;  
            clust(cn).maxDist = outMAXDistNew;
            clust(cn).IntCef = cefSUMNew;
			clust(cn).outMAXPointList = outMAXPointList;
			clust(cn).maxListNew = maxListNew;
			clust(cn).maxValueNew = maxValueNew;
