function [inp_str,inp_data] = getData_Statlog_Shuttle 
    fname = 'G:\WORKING\ACADEMIC\MATLAB\SKELETON\DataSets_Skeleton\Statlog (Shuttle)\shuttle_combined.trn';
    fid=fopen(fname,'r');
    % Missing Numeric Fields : ? --> 0 (As a category) 
    A=textscan(fid,'%f%f%f%f%f%f%f%f%f%s','Delimiter',',');
    size(A)
    Nrows = size(A{1},1);
    Ncols = size(A,2);
