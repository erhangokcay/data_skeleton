function [clust,numOfClust]=STREAM_SKELETON_VER4_nPNT_OPTIMIZED_V2(xy,var,T1,T2,randlist)
...
%         end        
    ...
    if ( isempty(list) )
        ...
        if ( length(list) == 1 ) % Join point to the group
            ...
            %clust(cn).Tvar = Dvar;
            clust(cn).DCirCef = combine(1).DCirCef;  
            clust(cn).maxDist =  combine(1).maxDist;
            clust(cn).IntCef = combine(1).IntCef;
			clust(cn).outMAXPointList = combine(1).outMAXPointList;
			clust(cn).maxListNew = combine(1).maxListNew;
			clust(cn).maxValueNew = combine(1).maxValueNew ;
...
% % %     end
    ...
    for n=1:size(clust,2);
        ...
        clust(n).maxList = unique(outMAXPointList','row')';
