function [inp_str,inp_data] = getData_Vehicle 
    fname = 'G:\WORKING\ACADEMIC\MATLAB\SKELETON\DataSets_Skeleton\Vehicle\vehicle_combined.data';
    fid=fopen(fname,'r');
    % Missing Numeric Fields : ? --> 0 (As a category) 
    A=textscan(fid,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%s','Delimiter',' ');
    size(A)
    Nrows = size(A{1},1);
    Ncols = size(A,2);
