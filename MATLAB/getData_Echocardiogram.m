function [inp_str,inp_data] = getData_Echocardiogram 
    fname = 'G:\WORKING\ACADEMIC\MATLAB\SKELETON\DataSets_Skeleton\Echocardiogram\echocardiogram.data';
    fid=fopen(fname,'r');
    % Missing Numeric Fields : ? --> 0 (As a category) 
    A=textscan(fid,'%f%f%f%f%f%f%f%f%f%f%f%f','Delimiter',',');
    size(A)
    Nrows = size(A{1},1);
    Ncols = size(A,2);
