...
function [out] = STREAM_SKELETON_INPUT_DATASET_OPTIMIZED_DEBUG1_1_RI ( inp_str, percent, flag, str, std1, T1, T2 )
...
%     inp_scaled=CPP_rescale(inp_data,-1,1);    
    if ( flag==false )   
        ...
        for n=1:Nlabels
            ...
            rd = floor(percent*NN); % MINIMUM 1 SAMPLE
            if ( rd==0)
                rd=1;
            end
...
for n=1:Nlabels
    ...
    [clust,numOfClust]=STREAM_SKELETON_VER4_nPNT_OPTIMIZED_V2_MEM_DEBUG1_1_RI(cl,std1,T1,T2, []);
