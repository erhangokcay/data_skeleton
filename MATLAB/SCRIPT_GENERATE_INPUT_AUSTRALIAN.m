...
data_Australian = getData_Australian;
%TList_IRIS_1=[1000,500,100,10,1,0.1, 10e-3,10e-4,10e-5,10e-8,10e-25,10e-50,10e-200,10e-350,10e-500]
% T=0 means original dataset. Copy by hand, not in the loop
TList_Australian_1=[1e5, 1e0, 1e-15,    1e-25,    1e-50,    1e-100,  1e-200];
TList = TList_Australian_1;
cef_var=0.1;
RdSelect = 1;
...
for T=1:length(TList)    
    for n=1:10
        ...
        inpstrAustralian(T,n) = STREAM_SKELETON_INPUT_DATASET_OPTIMIZED ( data_Heart, 0.2, false, [], 0.1, TList(T), TList(T) );
        % [outArr(n)] = STREAM_SKELETON_PERFORMANCE_ALL1 ( data_Cardiotocography, 0.2, false, 0, 0.1, TList(T), TList(T));
    end    
    save MATLAB_Australian_31_03_2021.mat
    % For debugging
