function [inp_str,inp_data] = getData_Congressional_Voting 
    fname = 'G:\WORKING\ACADEMIC\MATLAB\SKELETON\DataSets_Skeleton\Congressional Voting\house-votes-84.data';
    fid=fopen(fname,'r');
    % Missing Numeric Fields : ? --> 0 (As a category) 
    A=textscan(fid,'%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s','Delimiter',',');
    size(A)
    Nrows = size(A{1},1)
    Ncols = size(A,2)
