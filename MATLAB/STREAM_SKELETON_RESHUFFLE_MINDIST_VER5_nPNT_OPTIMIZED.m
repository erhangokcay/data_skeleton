function [clust,changed] = STREAM_SKELETON_RESHUFFLE_MINDIST_VER5_nPNT_OPTIMIZED(clust, numC, var, nPnt, T)

statInfo = CEF_CALCULATE_STATINFO(size(clust(1).xy),var,0);

    u1 = zeros(nPnt,1);
    u2 = zeros(nPnt,1);
    % Repeating this reshuffling while modified is not good.
    % It will consume the clusters until 1-point left. Repeat only once.
                ...
                %while ( modified )
                    ...
                    for k=1:nPnt % # of closest points from clust1  
                        ...
                        if ( len1 > k ) 
                           ...
                           xp = xy1(:,u1(1:k)); % xp
                           xtemp2 = xy2; % 
                           ind = CPP_Skeleton_Distance(xp,xtemp1,xtemp2);
                           if ( ind==1 )
                               xtemp2 = [xy2 xp];
                ...
                %while ( modified )
                    ...
                    for k=1:nPnt % # of closest points from clust1 
                        ...
                        if ( len2 > k ) 
                           ...
                           xtemp2 = xy2;
                           xtemp2(:,u2(1:k)) = []; % Delete from clust2
                           xp = xy2(:,u2(1:k)); % xp
                           xtemp1 = xy1; % 
                           ind = CPP_Skeleton_Distance(xp,xtemp2,xtemp1);
                           if ( ind==1 )
                               xtemp1 = [xy1 xp];                         
