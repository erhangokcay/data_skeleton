function [clust] = STREAM_SKELETON_RESHUFFLE_VER3(clust, var, T)
                ...
                while ( modified )
                    ...
                    if ( len1 > 1 && len2 > 1 ) 
                        for k=1:len1
                           ...
                           xtemp2 = [ xy2 xy1(:,k)];
                           DCirCefn = EXECUTE_CPP_CEF ( xtemp1, var, 0 );
                           DCirCefm = EXECUTE_CPP_CEF ( xtemp2, var, 0 );
                           % If the clusters are tigther
                           cond = lessT(DCirCefn,T) && lessT(DCirCefm,T);
                           if ( cond && (DCirCefn > clust(n).DCirCef && DCirCefm > clust(m).DCirCef) ) 
                                clust(n).xy = xtemp1;
                                clust(m).xy = xtemp2;
