function [inp_str,inp_data] = getData_Zoo 
    fname = 'G:\WORKING\ACADEMIC\MATLAB\SKELETON\DataSets_Skeleton\Zoo\zoo.data';
    fid=fopen(fname,'r');
    % Missing Numeric Fields : ? --> 0 (As a category) 
    A=textscan(fid,'%s%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%s','Delimiter',',');
    size(A)
    Nrows = size(A{1},1);
    Ncols = size(A,2);
